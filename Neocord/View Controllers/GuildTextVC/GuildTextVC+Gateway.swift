...
extension GuildTextViewController {
    ...
    func createMessage(_ message: Message) {
        ...
        if let messageID = message.id, let userID = message.author?.id, !messageIDsInStack.contains(messageID), self.channel?.id == message.channelID {
            DispatchQueue.main.async { [weak self] in
                guard let self = self else {return }
                self.messageStack.addArrangedSubview(MessageView(clientUser, message: message, guildTextChannel: self.channel))
